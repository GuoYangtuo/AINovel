...
todo:
log重置问题，保存重启后bug，弹幕说明（投票面板悬浮），自动滚动查看
接入直播功能：用户无需登录注册网站，在直播间里刷礼物+弹幕给出投票的选项即可进行投票，和正常用户一样参与投票并显示在@client/src/components/NovelLive.js 的底部投票记录里
